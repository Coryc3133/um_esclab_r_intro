...
+ Basics of what a package is, how to download one, and how to use one
...
We get an error telling us that the argument "x" is missing and has no default. Whenever you see this error, it means you are missing a required argument (i.e., an argument without a default). If we look at the documentation, you can see x is the data from which to calculate a mean.

Let's see what happens when we run `mean()` on the c1 column of the dataframe `df_2` we created above:
